...
def remove():
    with sqlite3.connect('database.db') as conn:
        ...
        categoryData = cur.fetchall()
    conn.close()
    return render_template('remove.html', data=data, categoryData=categoryData)
...
def displayCategory():
        ...
        with sqlite3.connect('database.db') as conn:
            cur = conn.cursor()
            cur.execute("SELECT products.productId, products.name, products.price, products.image, categories.name,products.description FROM products, categories WHERE products.categoryId = categories.categoryId AND categories.categoryId = ?", (categoryId, ))
            data = cur.fetchall()
            cur.execute('SELECT categoryId, name FROM categories')
            categoryData = cur.fetchall()
        conn.close()
...
def administration():
    ...
    if 'email' not in session:
        return redirect(url_for('root'))
    loggedIn, firstName, noOfItems = getLoginDetails()
    return render_template("administration.html", loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            if (password == oldPassword):
                try:
                    ...
                    conn.rollback()
                    msg = "Failed"
                return render_template("changePassword.html", msg=msg, profileData=profileData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
            else:
                msg = "Wrong password"
        conn.close()
        return render_template("changePassword.html", msg=msg, profileData=profileData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
    else:
        return render_template("changePassword.html",  profileData=profileData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
...
def loginForm():
    ...
    conn.close()
    if 'email' in session:
        return redirect(url_for('root'),categoryData=categoryData)
    else:
...
def checkout():
    ...
    if 'email' not in session:
        return redirect(url_for('root'))
    loggedIn, firstName, noOfItems = getLoginDetails()
    return render_template("checkout.html", loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
